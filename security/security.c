...
int security_vm_enough_memory_mm(struct mm_struct *mm, long pages)
{
	...
	lsm_for_each_hook(vm_enough_memory, rc,
		{
			if (rc <= 0) {
				cap_sys_admin = 0;
				goto out;
				    ...
				    struct fs_parameter *param)
{
	...
	lsm_for_each_hook(fs_context_parse_param, trc, {
		if (trc == 0)
			rc = 0;
			     ...
			     unsigned long *set_kern_flags)
{
	...
	lsm_for_each_hook(sb_set_mnt_opts, rc, {
			 ...
			 u32 __user *size, u32 flags)
{
	 ...
	 */
	LSM_LOOP_UNROLL(__CALL_HOOK, getselfattr, rc, {
